    ...
    for num_samples in [10**1, 3*10**1, 10**2, 3*10**2, 10**3, 3*10**3, 10**4, 3*10**4, 10**5, 3*10**5, 10**6]:
        ...
        for id_tt,tt in enumerate(tqdm(range(ttstart, ttend))):
            ...
            for epoch in range(epochs):
                ...
                os.makedirs(f"./trained/MLP_LBC/numsample={num_samples}_lr={lr}_epochs={epochs}/T_bp={T_LBC_range[tt]}", exist_ok=True)
                torch.save(model.state_dict(), f"./trained/MLP_LBC/numsample={num_samples}_lr={lr}_epochs={epochs}/T_bp={T_LBC_range[tt]}/epoch={epoch}.pt")
        
        print(f'\n---------------------***save file***------------------------\n')
        losses_cpu = losses.cpu().numpy()
        for id_tt,tt in enumerate(range(ttstart, ttend)):
            np.save(f"./trained/MLP_LBC/numsample={num_samples}_lr={lr}_epochs={epochs}/T_bp={T_LBC_range[tt]}/loss.npy", losses_cpu[id_tt,:])    
